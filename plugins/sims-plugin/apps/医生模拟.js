...
export class DoctorSimulation extends plugin {
  ...
  async startDoctorCareer(e) {
    ...
    userData.job = "医生";
    await saveAchievement(userId, {
      id: "doctor_join",
      title: "初入杏林",
      description: "成功就职成为一名医生",
      category: "职业-医生",
      meta: { hospitalLevel: userData.doctor.hospital.level }
    });
  ...
  async receivePatient(e) {
    ...
    // 统计并成就：首次接诊患者（兼容旧数据）
    doctorData.stats.patientsReceived = (doctorData.stats.patientsReceived || 0) + 1;
    if (doctorData.stats.patientsReceived === 1) {
      await saveAchievement(userId, {
        id: 'doctor_first_patient_received',
        title: '初诊启程',
        description: `你接诊了第一位患者：${newPatient.name}`,
        category: '职业-医生',
        meta: { patientId: newPatient.id, condition: newPatient.condition }
      });
    }
  ...
  async diagnosisPatient(e) {
    ...
    try {
      ...
      const match = msg.match(/#诊断\s*(\d+)/);
      if (!match) {
        e.reply("请使用正确格式：#诊断 患者ID，例如：#诊断 123456");
        return;
      }
      const patientId = match[1];
      const doctor = userData.doctor;
      const patient = doctor.currentPatients.find(p => String(p.id) === String(patientId));
      if (!patient) {
        e.reply(`未找到ID为 ${patientId} 的患者，请先接诊或确认ID是否正确。`);
        return;
      }
      if (!patient.treatment) patient.treatment = { diagnosis: null, medicines: [], surgery: { id: null, result: '待进行' }, notes: [] };
      if (patient.treatment.diagnosis) {
        e.reply(`患者 ${patient.name} 已完成诊断：${patient.treatment.diagnosis.name}`);
        return;
      }

      const disease = getDiseaseById(patient.disease.id);
      if (!disease) {
        e.reply("该患者的疾病信息缺失，暂无法进行诊断。");
        return;
      }

      // 计算诊断成功率（基于真实医生技能与疾病难度）
      const diagnosisSkill = doctor.skills.diagnosis || 0;
      const difficulty = disease.difficulty || 5;
      const baseChance = Math.min(Math.max(diagnosisSkill / 100, 0.05), 0.95);
      const difficultyFactor = Math.max(0.2, 1 - (difficulty - 5) * 0.08);
      const successChance = Math.max(0.05, Math.min(0.95, baseChance * difficultyFactor));
      const success = Math.random() < successChance;

      let confidence = Math.round(successChance * 100);
      let diagnosedDisease = disease;
      let diagnosisText = '';
      if (!success) {
        // 失败时使用真实疾病数据进行误诊
        const diseases = loadDiseases();
        const candidates = diseases.filter(d => d.id !== disease.id);
        if (candidates.length > 0) {
          diagnosedDisease = candidates[Math.floor(Math.random() * candidates.length)];
        }
        confidence = Math.max(30, Math.round(successChance * 70));
      }
      diagnosisText = success ? '诊断成功' : '诊断失败（可能误诊）';

      patient.treatment.diagnosis = {
        disease_id: diagnosedDisease.id,
        name: diagnosedDisease.name,
        confidence,
        difficulty: diagnosedDisease.difficulty || difficulty,
        symptoms: Array.isArray(diagnosedDisease.symptoms) ? diagnosedDisease.symptoms.slice(0, 6) : [],
        time: Date.now(),
        result: diagnosisText
      };

      // 更新满意度与医院声誉
      const satisfactionDelta = success ? Math.round(10 + (diagnosisSkill - difficulty * 10) * 0.05) : -Math.round(15 + difficulty * 2);
      patient.satisfaction = Math.max(0, Math.min(100, (patient.satisfaction || 0) + satisfactionDelta));
      doctor.hospital.reputation = Math.max(0, Math.min(100, doctor.hospital.reputation + (success ? 2 : -3)));

      // 技能与经验增长
      updateSkill(doctor, 'diagnosis', success ? 2 : 1);
      doctor.experience += success ? 40 : 20;
      const leveledUp = checkLevelUp(doctor);
      if (success) {
         doctor.stats.patientsTreated += 1;
         await saveAchievement(userId, {
           id: "firstDiagnosis",
           title: "初诊确立",
           description: "完成首次患者诊断",
           category: "职业-医生",
           meta: { patientId: patient.id, diseaseId: patient.treatment.diagnosis.disease_id }
         });
       } else {
         doctor.stats.mistakesMade += 1;
       }

      await saveUserData(userId, userData);

      const confidenceText = `${confidence}%`;
      e.reply(`患者【${patient.name}】(${patient.id}) ${diagnosisText}：${patient.treatment.diagnosis.name}，置信度 ${confidenceText}。当前满意度：${patient.satisfaction}，医院声誉：${doctor.hospital.reputation}。`);

     
       image(e, 'patients_list', { patients_count: doctor.currentPatients.length, patients: doctor.currentPatients });

    } catch (err) {
      console.error(`[医生模拟][diagnosisPatient]错误: ${err.message}`);
      e.reply("诊断过程中发生错误，请稍后再试。");
    }
  }
  ...
  async buyEquipment(e) {
    ...
    // 记录购买前设备数量，用于判断是否为首次购买
    const prevEquipCount = userData.doctor.hospital.equipment.length;
    ...
    // 成就：首次购买设备
    if (prevEquipCount === 0 && userData.doctor.hospital.equipment.length === 1) {
      await saveAchievement(userId, {
        id: 'doctor_first_equipment',
        title: '设备入门',
        description: `你的医院购入第一台设备：${equipment.name}`,
        category: '职业-医生',
        meta: { equipmentId }
      });
    }
  ...
  async performSurgery(e) {
    ...
    // 成就：首刀开刃（首例手术）
    if (userData.doctor.stats.surgeriesPerformed === 1) {
      await saveAchievement(userId, {
        id: 'doctor_first_surgery',
        title: '首刀开刃',
        description: `完成了人生中的第一台手术：${surgery.name}`,
        category: '职业-医生',
        meta: { surgeryId: surgeryId, difficulty: surgery.difficulty }
      });
    }
    // 成就：首例成功手术（去重由 saveAchievement 保证）
    if (isSuccess) {
      await saveAchievement(userId, {
        id: 'doctor_first_successful_surgery',
        title: '首例成功手术',
        description: `成功完成${surgery.name}`,
        category: '职业-医生',
        meta: { surgeryId: surgeryId, difficulty: surgery.difficulty }
      });
    }
    ...
    if (hasLeveledUp) {
      await saveAchievement(userId, {
        id: `doctor_level_${userData.doctor.level}`,
        title: `医道进阶 Lv.${userData.doctor.level}`,
        description: `医生等级提升到 Lv.${userData.doctor.level}，职称：${userData.doctor.rank}`,
        category: "职业-医生",
        meta: { rank: userData.doctor.rank }
      });
    }
    await saveUserData(userId, userData);
    await redis.set(`user:${userId}`, JSON.stringify(userData));
  ...
  async doctorTraining(e) {
    ...
    // 成就：首次培训完成
    if (userData.doctor.stats.trainingCompleted === 1) {
      await saveAchievement(userId, {
        id: 'doctor_first_training',
        title: '术业初修',
        description: `你完成了第一次${trainingType}培训，技能提升 ${oldSkill} → ${userData.doctor.skills[skillKey]}`,
        category: '职业-医生',
        meta: { skill: skillKey, gain: skillGain }
      });
    }
    const hasLeveledUp = checkLevelUp(userData.doctor);
    if (hasLeveledUp) {
  ...
  async medicalResearch(e) {
    ...
    if (action === "查看") {
      ...
      if (isCompleted) {
        ...
        // 成就：首次发表研究成果
        if (userData.doctor.research.publications === 1) {
          await saveAchievement(userId, {
            id: 'doctor_first_publication',
            title: '初登期刊',
            description: `发表了第一项研究成果：《${completedProject.name}》`,
            category: '职业-医生',
            meta: { field: completedProject.field }
          });
        }
  ...
  async handleMedicalEvent(e) {
    ...
    // 统计：累计处理医疗事件次数（兼容旧数据）
    userData.doctor.stats.eventsHandled = (userData.doctor.stats.eventsHandled || 0) + 1;
    // 成就：首次处理医疗事件
    if (userData.doctor.stats.eventsHandled === 1) {
      await saveAchievement(userId, {
        id: 'doctor_first_medical_event',
        title: '临危受命',
        description: `你首次处理医疗事件「${eventResult.event.name}」并获得结果：${eventResult.outcome}`,
        category: '职业-医生',
        meta: { eventName: eventResult.event.name, outcome: eventResult.outcome }
      });
    }
    const hasLeveledUp = checkLevelUp(userData.doctor);
    if (hasLeveledUp) {
  ...
  async upgradeHospital(e) {
    ...
    // 成就：首次升级医院（等级升至2时触发）
    if (userData.doctor.hospital.level === 2) {
      await saveAchievement(userId, {
        id: 'doctor_first_hospital_upgrade',
        title: '医院升级·初阶',
        description: `你的医院首次升级为【${nextLevel.name}】`,
        category: '职业-医生',
        meta: { newLevel: userData.doctor.hospital.level }
      });
    }
  ...
  async setSpecialty(e) {
    ...
    if (!oldSpecialty) {
      await saveAchievement(userId, {
        id: 'doctor_choose_specialty',
        title: '术业有专攻',
        description: `你选择了专科方向：${specialty}`,
        category: '职业-医生',
        meta: { specialty }
      });
    }
