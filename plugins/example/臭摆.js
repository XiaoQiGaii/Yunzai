...
export class example extends plugin {
  ...
  async randomimg(e){
    const path = process.cwd() + '/resources/baibai/';
  ...
  async _downloadAndSave(e) {
    const folder = process.cwd() + '/resources/baibai/';
    const folder1 = process.cwd() + '/resources/qunyou/';
    try {
    ...
    try {
      ...
      const savePath1 = `${folder}/${fileName}`;
      fs.writeFileSync(savePath, buf);
      fs.writeFileSync(savePath1, buf);
