...
export const autoReplySchema = (): ConfigSchemaType[] => [
    ...
    },
    {
        ...
        label: "群聊AI ApiKey",
        bottomHelpMessage:
            "填写AI接口和ApiKey、确保主动群聊开关开启、并选择正确的群聊AI接口类型、并确保ApiKey为开启状态后，务必先保存并刷新页面，否则模型无法选择！",
        component: "GSubForm",
        componentProps: {
            multiple: true,
            schemas: [
                ...
                },
                {
                    ...
                    required: true,
                    bottomHelpMessage:
                        "请求时，会遍历尝试使用这些API Key，直到成功或全部失败",
                },
                {
                    field: "enabled",
                    label: "启用状态",
                    ...
                    bottomHelpMessage:
                        "是否启用该API Key，若关闭则不会被遍历。不管是还是否，都要点一下才能提交",
                    required: true,
                }
            ],
        }
    },
    {
        ...
        label: "群聊AI接口类型",
        bottomHelpMessage:
            "勾选视觉可支持图片识别。在勾选前请确保所选择的接口和模型支持相关功能。选择该项后，先保存并刷新页面，否则模型无法选择！",
    ...
    },
    {
        ...
        componentProps: {
            defaultValue:
                "如果群聊接口不支持视觉功能，或视觉功能让你不太满意，想用其它的视觉AI辅助处理时，可以开启该项\n" +
                "费用比较：\n" +
