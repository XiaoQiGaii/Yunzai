...
export class douBao extends plugin {
    ...
    constructor() {
        ...
        if (config.douBao.useImageStyle || config.douBao.useImageImitate) {
            this.fetchImageService = getServiceApi(config.douBao.imageService.host, config.douBao.imageService.accessKeyId, config.douBao.imageService.secretAccessKey, "POST", config.douBao.imageService.action, config.douBao.imageService.version, config.douBao.imageService.region, config.douBao.imageService.service);
        }
        /*
        ...
        */
        if (config.douBao.useSongGenerate) {
            this.fetchSongGenerate = getServiceApi(config.douBao.songService.host, config.douBao.songService.accessKeyId, config.douBao.songService.secretAccessKey, "POST", config.douBao.songGenerate.action, config.douBao.songGenerate.version, config.douBao.songService.region, config.douBao.songService.service);
            this.fetchSongQuery = getServiceApi(config.douBao.songService.host, config.douBao.songService.accessKeyId, config.douBao.songService.secretAccessKey, "POST", config.douBao.songGenerate.queryAction, config.douBao.songGenerate.queryVersion, config.douBao.songService.region, config.douBao.songService.service);
            this.fetchBgmGenerate = getServiceApi(config.douBao.songService.host, config.douBao.songService.accessKeyId, config.douBao.songService.secretAccessKey, "POST", config.douBao.bgmGenerate.action, config.douBao.bgmGenerate.version, config.douBao.songService.region, config.douBao.songService.service);
        }
        // --------------------------------------------------------------------------------------------------------------------
    }
    async help(e) {
    ...
    // ---------------------------------------------------- 图片生成 ----------------------------------------------------
    static imageGenerateMap = {
        "doubao-seedream": {
            "text": "doubao-seedream-3-0-t2i-250415"
        }
    };
    static getImageGenerateModel(msgList) {
    ...
    static getVideoGenerateModel(msgList) {
        if (msgList.images.length > 1) {
    ...
    async videoGenerate(e) {
        ...
        for (var i = 0; i < msgList.content.length; i++) {
            ...
            if (content.type == "text") {
                request.options.body.content.push({
                    type: "text",
                    text: content.text,
                });
            }
                ...
                request.options.body.content.push(content);
            }
        }
        request.options.body = JSON.stringify(request.options.body);
        let response = await fetch(request.url, request.options);
    ...
    */
    ...
    async songGenerate(e) {
        ...
        for (var i = 1; i < strPreList.length; i++) {
            //选项中可能含有空格，所以要做预处理
            if (StringUtils.startsWithStrs(strPreList[i], options))
                strList.push(strPreList[i]);
            else
                strList[strList.length - 1] = strList.at(-1) + " " + strPreList[i];
        }
        for (var i = 0; i < strList.length; i++) {
            if (strList[i].startsWith("-g"))
                body.Genre = strList[i].replace("-g", "");
            else if (strList[i].startsWith("-m"))
                body.Mood = strList[i].replace("-m", "");
            else if (strList[i].startsWith("-s"))
                body.Gender = strList[i].replace("-s", "");
            else if (strList[i].startsWith("-t"))
                body.Timbre = strList[i].replace("-t", "");
            else if (strList[i].startsWith("-d"))
                body.Duration = parseInt(strList[i].replace("-d", ""));
    ...
    createSongGenerateTaskThread(e, id, successHandler, failHandler) {
        var body = {
            TaskID: id,
        };
        var taskThread = setInterval(async () => {
    ...
    async bgmGenerate(e) {
        ...
        for (var i = 0; i < strList.length; i++) {
            if (strList[i].startsWith("-g"))
                body.Genre = strList[i].replace("-g", "").split(",");
            else if (strList[i].startsWith("-m"))
                body.Mood = strList[i].replace("-m", "").split(",");
            else if (strList[i].startsWith("-i"))
                body.Instrument = strList[i].replace("-i", "").split(",");
            else if (strList[i].startsWith("-t"))
                body.Theme = strList[i].replace("-t", "").split(",");
            else if (strList[i].startsWith("-d"))
                body.Duration = parseInt(strList[i].replace("-d", ""));
            else if (strList[i].startsWith("-p")) {
    ...
    createBGMGenerateTaskThread(e, id, successHandler, failHandler) {
        var body = {
            TaskID: id,
        };
        var taskThread = setInterval(async () => {
    ...
    async handleBGMFailed(e, response) {
        // 处理失败
        var error = response.error;
        var message = error.message;
        var code = error.code;
    ...
    async handleBGMGenerateCompleted(e, response) {
        const audioUrl = response.Result.SongDetail.AudioUrl;
        const res = await fetch(audioUrl);
        if (!res.ok)
            throw new Error(`HTTP error! status: ${res.status}`);
